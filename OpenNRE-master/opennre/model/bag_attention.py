...
class BagAttention(BagRE):
    ...
    def __init__(self,  num_class, rel2id):
        """
        Args:
    ...
    def infer(self, bag):
        ...
        tokens = []
        for item in bag:
            token = self.tokenizer(item['token'], is_token=True, padding=True)
            tokens.append(token)
        tokens = torch.cat(tokens, 0) # (n, L)
        scope = torch.tensor([[0, len(bag)]]).long() # (1, 2)
        bag_logits = self.forward(None, scope, tokens, train=False).squeeze(0) # (N) after softmax
    ...
    def forward(self, label, scope, token, train=True):
        """
        Args:
