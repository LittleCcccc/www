...
class SentenceREDataset(data.Dataset):
    ...
    def eval(self, pred_result, use_name=False):
        ...
        micro_p = float(correct_positive) / (float(pred_positive) + 1)
        micro_r = float(correct_positive) / (float(gold_positive) + 1)
...
class BagREDataset(data.Dataset):
    ...
    def __init__(self, path, rel2id, tokenizer, entpair_as_bag=False, mode=None):
        ...
        for i in range(self.__len__()):
            self.mydata.append(self.mygetitem(i))
            if(i % 1000 == 0):
                print(i)
...
class MyDataset(data.Dataset):
    ...
    def __init__(self, path, rel2id, entpair_as_bag=False, mode=None):
        """
        Args:
            path: path of the input file
            rel2id: dictionary of relation->id mapping
        ...
        # Construct bag-level dataset (a bag contains instances from same article)
        if mode == None:
            self.bag_scope = []
            self.name2id = {}
            self.bag_name = []
            self.facts = {}
            for idx, item in enumerate(self.data):
                fact = (item['article_id'],item['relation'])
                self.facts[fact] = 1
                if entpair_as_bag:
...
def MyLoader(path, rel2id, batch_size,
                shuffle, entpair_as_bag=False, num_workers=0,
                collate_fn=BagREDataset.collate_fn):
    dataset = BagREDataset(path, rel2id, tokenizer, entpair_as_bag=entpair_as_bag)
    data_loader = data.DataLoader(dataset=dataset,
