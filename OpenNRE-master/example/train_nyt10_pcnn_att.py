...
rel2id = json.load(open('exdata/rel2id.json'))
...
#                                              word2vec=word2vec,
#                                              dropout=0.5)
model = opennre.model.BagAttention(len(rel2id), rel2id)
framework = opennre.framework.BagRE(
    train_path='exdata/train.txt',
    val_path='exdata/val.txt',
    test_path='exdata/val.txt',
    model=model,
    #ckpt=ckpt,
    batch_size=160,
    max_epoch=60,
